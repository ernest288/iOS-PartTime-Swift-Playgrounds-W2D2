...
travel(name: "Ernest", distance: 2.5)
...
func speedDescription (kph: Double) -> String {
    var result : String
    if kph == 0  {
        result = "We're standing still"
    } else if kph > 30 {
        result = "Whoa slow down buddy, \(kph) km/h is too fast for this school zone!"
    } else {
        result = "We're going \(kph) km/h"
    }
    return result
}
